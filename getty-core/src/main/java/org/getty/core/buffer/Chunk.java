...
public final class Chunk {
     ...
     * 当前可用的缓冲区
     */
    private List<ChunkPage> availableChunkPages = new LinkedList<>();
    /**
     * 当前缓存页总的缓冲区
     */
    ...
    Chunk(int size, boolean direct) {
        ...
        this.direct = direct;
        ChunkPage chunkPage = new ChunkPage(this, buffer, buffer.position(), buffer.limit());
        availableChunkPages.add(chunkPage);
    }
    ...
    public ChunkPage allocate(final int size) {
        ...
        try {
            //当前空闲的虚拟缓冲区
            Iterator<ChunkPage> iterator = availableChunkPages.iterator();
            while (iterator.hasNext()) {
                ...
                if (remaining == size) {
                    ...
                    buffer.position(freeChunk.getPosition());
                    //创建新的子缓冲区
                    ChunkPage bufferChunk = new ChunkPage(this, buffer.slice(), buffer.position(), buffer.limit());
                    //当前被分割的子缓冲区重新定位
                    freeChunk.setPosition(buffer.limit());

                    //大小不匹配，抛个异常,这里理论上是不会出问题的。
     ...
     * @param chunkPage
     */
    void clean(ChunkPage chunkPage) {
        if (chunkPage.getChunk() == null) {
            return;
        }
        //尝试获取锁
        if (!lock.tryLock()) {
    ...
    private void clean0(ChunkPage chunkPage) {
        int index = 0;
        Iterator<ChunkPage> iterator = availableChunkPages.iterator();
        ...
        while (iterator.hasNext()) {
            ...
            if (freeBuffer.getPosition() == chunkPage.getLimit()) {
                freeBuffer.setPosition(chunkPage.getPosition());
                return;
            }
            //反之如果可用缓冲区的limi刚好等于清理的position。则移动到清理的缓冲区limit处，使着连贯起来
            if (freeBuffer.getLimit() == chunkPage.getPosition()) {
                freeBuffer.setLimit(chunkPage.getLimit());
                if (iterator.hasNext()) {
                    ...
                    if (next.getPosition() == freeBuffer.getLimit()) {
                        ...
                        throw new IllegalStateException("chunkPage clean Exception");
                    }
                }
                return;
            }
            if (freeBuffer.getPosition() > chunkPage.getLimit()) {
                availableChunkPages.add(index, chunkPage);
                return;
            }
            index++;
        }
    ...
    public String toString() {
        return "Chunk{availableChunkPages=" + availableChunkPages + '}';
    }
}
